...
表示月、選択日、開いているパネル、編集中の予定IDはURL queryにも反映する。初期表示とブラウザの戻る・進むではqueryを検証して画面状態へ復元し、不正な月・日付・存在しない予定IDは安全な既定画面へ戻す。query更新時は本アプリが所有しないパラメータとhashを維持する。
...
- 日曜始まりの7列
- 今日と選択日を区別
- 予定がある日は、日付と最大3個の色ドットをひとまとまりで中央配置
- 表示月と前月・翌月の移動操作はカレンダー上部にまとめる
- 手動再読み込みと設定はページ上部のヘッダーに置く
- 380px幅で横スクロールなし
- 選択日と今日を支援技術へ通知する
...
- フォーム要素は16px以上
- 入力エラーは該当欄の直下に表示し、最初の不正欄へフォーカスする
- モバイルではダイアログを開いた直後にソフトウェアキーボードを出さない
...
- エクスポート結果のクリップボードコピー
- 正規月キーだけのマージインポート
- 非同期処理中は対象操作名と進行状態を表示し、重複操作を防ぐ
- 失敗理由は発生元の操作付近に、次の操作とともに表示する
- モバイルでは設定画面を全画面表示し、ヘッダーとフッターを残して内容だけスクロールする

### アクセシビリティと動作設定

- 本文へのスキップリンクとキーボードフォーカス表示を提供する
- 保存状態の変化はlive regionで通知する
- タッチ操作対象は44px相当を確保し、モーダル内のスクロール連鎖を抑止する
- `prefers-reduced-motion` では装飾的なアニメーションとtransitionを抑制する
